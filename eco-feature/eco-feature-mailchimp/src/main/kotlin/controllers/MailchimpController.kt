...
class MailchimpController(
    ...
    fun getMembers(page: Pageable): ResponseEntity<List<MailchimpList>> =
        mailchimpClient.getLists(page)
            .let {
    ...
    fun getMembers(
        ...
        page: Pageable,
    ): ResponseEntity<List<MailchimpMember>> =
        mailchimpClient.getMembers(listId, page)
            .let {
                val headers = HttpHeaders()
                headers.set("x-page", page.pageNumber.toString())
                headers.set("x-total", it.totalElements.toString())
    ...
    fun getInterestCategories(
        @PathVariable listId: String,
    ): List<MailchimpInterestCategory> = mailchimpClient.getInterestsCategories(listId)
    ...
    fun postWebhook(
        ...
        val event =
            MailchimpWebhookEvent(
                ...
                interests =
                    formData.getFirst("data[merges][INTERESTS]")
                        ?.split(",")
                        ?.map { it.trim() }
                        ?.toSet()
                        ?: setOf(),
