...
class MemberService(
    ...
    fun findAll(pageable: Pageable): Iterable<Member> =
        memberRepository
            .findAll(pageable)

    fun findAll(
        specification: Specification<Member>,
        pageable: Pageable,
    ): Page<Member> =
        memberRepository
    ...
    fun count(specification: Specification<Member>): Long =
        memberRepository
            .count(specification)
    ...
    fun findByStatus(status: MemberStatus) =
        memberRepository
            .findByStatus(status)

    fun create(input: Member): Member =
        input
            .save()
    ...
    @Transactional
    fun merge(
        mergeMemberIds: List<UUID>,
        newMember: Member,
    ): Member {
        val mergeMembers =
            memberRepository.findByUuids(mergeMemberIds)
                .map {
                    it.copy(
    ...
    private fun Member.publish(function: (member: Member) -> MemberEvent): Member =
        apply {
